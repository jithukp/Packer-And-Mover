...
public class view_worker extends AppCompatActivity {
    ListView lv;
                        ...
                        try {
                            ...
                            if (jsonObj.getString("status").equalsIgnoreCase("ok")) {
                                ...
                                for(int i=0;i<js.length();i++) {
                                    ...
                                    id[i] = u.getString("u_id");//dbcolumn name
                                    name[i] = u.getString("first_name")+" "+u.getString("last_name");//dbcolumn name
                                    place[i] = u.getString("place");//dbcolumn name
                                    district[i] = u.getString("district");//dbcolumn name
                                    phone[i] = u.getString("phone_no");//dbcolumn name
                                }
                                lv.setAdapter(new custom_worker(getApplicationContext(),id, name, place, district, phone));//custom_view_service.xml and li is the listview object
